...
const HeroBanner = ({ contentSet }) => {
  ...
  return (
    <div
      className="h-banner-mobile md:h-banner-desktop bg-cover bg-center bg-no-repeat flex items-end p-4 md:p-8 relative"
      style={{ backgroundImage: `linear-gradient(to top, rgba(26, 32, 44, 0.95) 0%, rgba(26, 32, 44, 0.8) 20%, rgba(26, 32, 44, 0.3) 60%, transparent 100%), url(${banner_url || defaultBanner})` }}
    >
      <div className="max-w-xl z-10">
        ...
        <Link
          to={`/set/${set_id}`}
          className="bg-brand-primary hover:bg-brand-secondary text-white font-semibold py-2 px-4 md:py-3 md:px-6 rounded-lg text-sm md:text-base transition duration-300 ease-in-out transform hover:scale-105 focus:outline-none focus:ring-2 focus:ring-brand-primary focus:ring-opacity-50"
        >
          {t('heroActionText')}
        </Link>
      </div>
